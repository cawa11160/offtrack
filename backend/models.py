...
class Notification(Base):
    ...
    read_at: Mapped[object | None] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
    created_at: Mapped[object] = mapped_column(DateTime(timezone=True), server_default=func.now(), index=True)


class UploadDiscoveryControl(Base):
    __tablename__ = "upload_discovery_controls"
    ...
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), index=True)
    discovery_paused: Mapped[bool] = mapped_column(Boolean, default=False, index=True)
...
Index("ix_upload_discovery_controls_user_paused", UploadDiscoveryControl.user_id, UploadDiscoveryControl.discovery_paused)
