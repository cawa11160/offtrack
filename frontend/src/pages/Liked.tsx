      ...
      <section>
        ...
        <div className="divide-y divide-border">
          {likedSongs.map((song, index) => (
            ...
            >
              {/* LEFT FIXED CELL – no shifting on hover */}
              <div className="hidden md:block relative w-4 h-4 text-sm text-muted-foreground">
                <span className="absolute inset-0 grid place-items-center opacity-100 group-hover:opacity-0 transition-opacity">
                  {index + 1}
                </span>
              ...
              <div className="flex items-center gap-3 min-w-0">
                <div className="relative w-12 h-12 shrink-0">
                  ...
                  {/* Play overlay on image (does NOT move layout) */}
                  <button
                    type="button"
                    aria-label={`Play ${song.title}`}
                    className="absolute inset-0 grid place-items-center rounded bg-black/35 opacity-0 group-hover:opacity-100 transition-opacity"
                    // onClick={() => ...play song...}
                  >
